...
# ---------------------------------------------------------------- 출력 필터 (헌법 ②)
# 가드 두 종류 — 섞지 않는다(시범 제조 2026-09-17에서 드러남):
#   ① 고정 문구형(여기 _VERDICT_PATTERNS·filter_output): "법적 효력이 있다"처럼 문장 자체가 금지인 것 → 정규식 치환
#   ② 판단형(날조·창작 금지처럼 «내용»을 봐야 하는 것): 정규식으로 못 잡는다 → 프롬프트 규칙(skills/*.md) + engine 의 self_check(원문 대조·[확인 필요] 태깅) + 사람 승인 게이트 3중
_VERDICT_PATTERNS = [
