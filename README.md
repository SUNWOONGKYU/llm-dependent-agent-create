...
이 스킬은 **`mbo-천상`(공개명 `mbo-skill`, 호출 `/mbo`)에 필수 의존**한다. Phase 2 목표서 양식·PO 승인 게이트·MBO 파일 저장·Phase 8 결과 보고가 전부 그 스킬에서 온다. 없으면 착수 전 자동 설치를 시도한다(Phase 0-0). SKILL.md 본문의 `mbo-천상`·`/mbo-천상` 표기는 전부 이 스킬을 가리킨다 — 공개 저장소·설치 폴더명은 `mbo-skill`·`mbo`, 실제 호출은 `/mbo`이다.
